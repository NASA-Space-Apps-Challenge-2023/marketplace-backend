...
async def add_keywords_bulk(keywords: KeywordsList = Body(...)):
    ...
    return ErrorResponseModel("An error occurred", 400, "There was an error adding the keywords.")
...
async def delete_keyword_data(id: str):
    ...
    if deleted_keyword:
        return ResponseModel(
            "Keyword with ID: {} removed".format(id),
            "Keyword deleted successfully"
        )
    return ErrorResponseModel(
        "An error occurred", 404, "Keyword with id {0} doesn't exist".format(
            id)
    )
